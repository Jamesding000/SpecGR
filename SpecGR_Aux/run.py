...
from models.SpecGR.SpecGR_clean import SpecGRAuxForRec
...
def load_unisrec_data_and_model(config, device, eval_mode='test'):
    ...
    draft_test_in_sample_dataloader, draft_test_cold_start_dataloader = get_dataloaders(
        ...
        train_batch_size=config['SpecGR']['inference']['eval_batch_size'],
        eval_batch_size=config['SpecGR']['inference']['eval_batch_size'],
...
def load_tiger_data_and_model(config, device, eval_mode='test'):
...
def run_SpecGR_Aux(config, params, device, eval_mode='test'):
    # Load UniSRec data and model
    unisrec, item_embeddings, draft_test_in_sample_dataloader, draft_test_cold_start_dataloader = load_unisrec_data_and_model(config, device, eval_mode)

    # Load TIGER data and model
    genrec, semantic_ids, target_test_in_sample_dataloader, target_test_cold_start_dataloader = load_tiger_data_and_model(config, device, eval_mode)
    ...
    specGR = SpecGRAuxForRec(genrec, draft_model, config, semantic_ids, params)

    # Initialize the evaluator
    evaluator = SpecGRAuxEvaluator(specGR, ks=config['eval_ks'])

    # Evaluate the model
    cold_start_result = evaluator.evaluate(test_cold_start_dataloader, device=device, item_embeddings=item_embeddings, semantic_ids=semantic_ids)
    in_sample_result = evaluator.evaluate(test_in_sample_dataloader, device=device, item_embeddings=item_embeddings, semantic_ids=semantic_ids)
...
if __name__ == "__main__":
    ...
    run_SpecGR_Aux(config, params, device, eval_mode='test')