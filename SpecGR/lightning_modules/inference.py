...
# from models.SpecGR.SpecGR_single import SpecGRForRec
...
class SpecGRForRecLightningModule(L.LightningModule):
    ...
    def _load_checkpoint(self) -> None:
        self.model.load_state_dict(torch.load(self.saved_model_path, map_location='cuda:0'))
        print(f'Loaded model checkpoint from {self.saved_model_path}.')

        self.init_item_embeddings()
        self.item_embeddings.load_state_dict(torch.load(self.saved_embedding_path, map_location='cuda:0'))
    ...
    def _print_params_to_console(self) -> None:
        if not dist.is_initialized() or (dist.is_initialized() and dist.get_rank() == 0):
            ...
            table.add_row('draft_size', str(self.model.draft_size))
            table.add_row('threshold', str(self.model.threshold))
            table.add_row('num_beams', str(self.model.num_beams))
    ...
    def on_test_epoch_end(self) -> None:
        ...
        self.log_dict(avg_metrics, on_step=False, on_epoch=True, prog_bar=False, sync_dist=True)